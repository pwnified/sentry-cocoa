...
- (SentryEvent *)convertReportToEvent {
    ...
    // Apparently console_log is disabled in multiple ways, so add the console text line by line to the
    // event.extra until we get this figured out. This appears on the server, sorted perfectly by key.
    static const int kMaxConsoleLines = 200;
    static const int kRequiredFirstLines = 15;
    NSMutableDictionary *extra = [NSMutableDictionary dictionaryWithDictionary:event.extra];
    NSArray *lines = self.report[@"debug"][@"console_log"];
